...
| | What it does | Needs |
| --- | --- | --- |
| `yarn test:layout` | 141 checks over every room size, shape and hundreds of seeds: anchors clear of the door lanes and of each other, every arrangement legal, every shipped template one the game will draw whole, the descent's rules never gentler with depth, the pan of a sound correct from all 360 headings, the Warden outrun by every sprint in the game and no ground in the arena its arms do not reach | nothing |
| `yarn test:smoke` | 111 checks driving the real game: the whole loop from menu to victory and to death, the economy, the Warden and the noise it hears, the locked vault, the three puzzles won, lost and walked away from, the arena's arms, the records, the editor and the content pipeline | a dev server on 5199, Chromium |
