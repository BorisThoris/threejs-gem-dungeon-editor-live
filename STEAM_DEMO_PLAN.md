...
always a cost and never a lie. Then ten deeds - sixteen since the satchel resolved - with a real seam to Steam's
...
What the numbers say today: the worst room costs 55 draw calls and 2,364
triangles against a written budget of 72 and 3,400; a first visit downloads
1.05 MB gzipped; a floor is 8 to 16 rooms depending on depth and takes 19 to
22 seconds to cross at a walk. Thirty-four percent of the rooms in a run are
rooms somebody made, up from eight.
...
- **Nothing was cut.** The refinement runs each added something and none
  removed a system. A demo that is too full is a real failure mode, and
  nothing here tests for it.
- **Whether any encounter should be unavoidable.** Run 29 made the set
  pieces worth walking into, which was the half of PLAYTEST §22's problem
  that was a number nobody set rather than a design choice. Whether a
  player should be able to finish the demo without meeting one at all is
  still a decision for a person.
...
| 10 | **The one you cannot escape if you linger.** Every floor heats, and the last band is where it stops putting up with you. When it does something wakes that has no room, no alarm and no lure - a ghost body, through walls and spikes and wards, faster than a walk, and it does not leave. A bomb stalls it. The HUD names how the floor feels rather than counting it down - quiet, noticed, looking, awake, and then IT KNOWS WHERE YOU ARE - and the score's heartbeat comes in under it. Risk against reward: the treasure room you have not opened, or the door. | Spelunky's ghost and Barony's minotaur, and the missing shape of a run. |
...
| 22 | **One teacher for every system.** The Warden's systems had first-time lines and the ten loops' did not: a sprung plate, a pit, a grate, a draft, a wisp, a moth, bats, a rat in a snare, a burst barrel, wings and the Keeper all happened in silence. One table, one voice, twenty-eight rows, and floor blurbs that name what is new where it is new. | Every rule the player cannot see is said once, by the one module that says things. |
| 23 | **The first room of a floor is a sanctuary.** Nothing that hunts comes into the room a floor starts you in while the player is still standing there - the Warden's step, the Harrier's waking and the thief's arrival all ask one predicate - and it is an ordinary room the moment they leave it and come back. The floor reaching its last band ends it too, so the stairs are not a place to wait. | The first breath of a floor is free because it is only the first. |
| 24 | **The second before a hit.** The three things that take a life by touching you show it coming: the Warden rises and its eyes swell over its last three strides, the Keeper's halberd comes down as you enter its reach, the Harrier tips into its dive. Each publishes one number, `tell`, that the body wears and the check reads. | The Warden's arrival grace was never a windup - it is spent crossing the room - so it hit with no notice at all. |
...
| 29 | **A set piece is worth walking into.** Run 28's own review named the worst property of this as a demo: the fastest route sees the least of it. Half of that was not a design question but a number nobody set - a solved tome, trial or plate paid one gem, the same as the gem loose on the same room's floor. A set piece pays two now, from one owner all three rooms ask, so robbing a room pays one and answering it pays three. | A choice between one gem and one gem is not a choice. |
