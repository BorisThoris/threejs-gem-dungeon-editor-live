...
import { lightIsShowing, useRun } from "../state/run";
import { wispAt } from "./lamplighter";

/**
 * Whether the wisp is out, decided once a frame from the one fact it
 * follows: the player's light is showing. Written to the store
...
export function WispDriver() {
  useFrame(() => {
    ...
    const out = run.phase === "playing" && !!run.dungeon && lightIsShowing(run);
