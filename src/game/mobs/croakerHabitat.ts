...
import { WATER_DRAIN_SECONDS } from "../worldbuilding/watercourse";
...
export function croakerHabitats(room: Room, spots: Spot[], seed: number): CroakerHabitat[] {
  ...
  const banks = terrain.deposits.filter(tile => tile.bank && !tile.slope?.some(slope => Math.abs(slope) > 1e-6))
    .map(tile => ({ x: tile.position[0], z: tile.position[2] }));
  const beds = ["flooded", "mossy", "fungal"].includes(terrain.biome) ? terrain.deposits
    .filter(tile => !tile.bank && !tile.slope?.some(slope => Math.abs(slope) > 1e-6))
    .map(tile => ({ x: tile.position[0], z: tile.position[2] })) : [];
