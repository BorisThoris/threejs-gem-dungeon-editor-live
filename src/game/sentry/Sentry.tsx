...
import { playerLightIn } from "../ladder/sight";
...
export function Sentry({ position, phase }: { position: Vec3; phase: number }) {
  ...
  useFrame((state) => {
     ...
     */
    /**
     * Its row is [bright] at 0.50. Sample the light on the player, including
     * nearby fixtures and the wisp, rather than exposing every dark corner
     * because a light exists somewhere in the room. Sound never changes it.
     */
    const lit = !!room && playerLightIn(room.id) >= SUSCEPTIBILITY.sentry.answers.bright!;
