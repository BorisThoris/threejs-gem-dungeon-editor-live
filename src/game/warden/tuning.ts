...
/**
 * How roused the Warden is, from the floor's alarm. One place, so the
 * driver, the component, the audio and the HUD all agree about what a given
 * alarm level means.
 */
...
export function behaviourFor(alarm: number): WardenBehaviour {
  ...
  return {
    ...
    hunts: alarm >= ALARM_HUNTS_AT,
  };
}

/** What the HUD calls the current state of the floor. */
export function alarmLabel(alarm: number): string {
