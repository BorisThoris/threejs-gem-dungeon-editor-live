...
export const TERRAIN_GRAMMAR: Record<BiomeId, { name: string; description: string }> = {
  ...
  tallow: { name: "Votive runs", description: "Cooled wax fills paired rendering channels while every fourth cross-cut preserves the processional crossing." },
};
...
export function chamberTerrainCell(room: Room, biome: BiomeId, x: number, z: number): TerrainCell {
  ...
  switch (biome) {
    ...
    case "tallow": {
      const crossCut = row % 4 === 0;
      deposit = ax > half * 0.32 && border < half * 0.52 && !crossCut;
      break;
    }
  }
...
export function galleryTerrainCell(biome: BiomeId, along: number, across: number, width: number, ramp: boolean): TerrainCell {
  ...
  if (biome === "tallow") return Math.abs(across) > width * 0.26 && Math.floor(along / 1.5) % 4 !== 0 ? "deposit" : "paving";
