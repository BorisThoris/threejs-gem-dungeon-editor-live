...
import { CanvasTexture, NearestFilter, type Group, MeshStandardMaterial } from "three";
...
import { watercourseBlocks, waterLevel, waterStation, WATERWAY_NAMES, WATER_CACHE_GEMS } from "./watercourse";
...
export function Watercourse({ room }: { room: Room }) {
  ...
  const water = useMemo(() => {
    ...
    material.onBeforeCompile = shader => {
      ...
      shader.fragmentShader = shader.fragmentShader.replace("#include <color_fragment>",
        "#include <color_fragment>\nfloat bands = step(0.76, fract((flowUV.x + flowUV.y) * 12.0 - floor(flowTime * 5.0) * 0.08)); diffuseColor.rgb *= 0.9 + bands * 0.4;");
    };
    return material;
  }, []);
  ...
  return <group>
    ...
    <group>
      {blocks.map((b, i) => {
        const dirs = [room.waterway!.upstream, room.waterway!.downstream].filter(Boolean);
        const incoming = !!room.waterway!.upstream && i === 0;
        const reverse = (dirs[i] === "west" || dirs[i] === "south") !== incoming;
        return <mesh key={i} position={[b.position[0], GROUND_Y + 0.044, b.position[2]]} rotation={[-Math.PI / 2, 0, reverse ? Math.PI : 0]}>
    ...
    {room.waterway.downstream && <group rotation={[0, DIR_YAW[room.waterway.downstream], 0]}>
      {[2, 4].map(z => <group key={z} position={[0, GROUND_Y + 0.052, -z]}>
        {[-1, 1].map(sign => <mesh key={sign} position={[sign * 0.22, 0, 0]} rotation={[0, sign * Math.PI / 4, 0]}>
          <boxGeometry args={[0.08, 0.016, 0.65]} /><meshStandardMaterial color="#d8b873" emissive="#695027" emissiveIntensity={0.35} />
        </mesh>)}
      </group>)}
    </group>}
    {station && <>
