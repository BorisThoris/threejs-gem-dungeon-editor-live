...
try{
const page=await browser.newPage();await page.goto('http://127.0.0.1:5222/');await page.locator('[data-testid="menu-start"]').click();await page.waitForFunction(()=>window.__run?.getState().phase==='playing'&&!window.__run.getState().transitioning);
...
console.log(JSON.stringify(await page.evaluate(()=>{const rows=[];window.__scene.traverse(o=>{if(!o.geometry)return;let parent=o;while(parent&&!parent.name)parent=parent.parent;rows.push({name:parent?.name,type:o.geometry.type,count:o.count??1,triangles:(o.geometry.index?.count??o.geometry.attributes.position.count)/3*(o.count??1),color:o.material?.color?.getHexString()});});return {perf:window.__perf, room:window.__run.getState().dungeon.rooms.find(r=>r.id==='room_6'),rows:rows.sort((a,b)=>b.triangles-a.triangles).slice(0,25)};}),null,2));
