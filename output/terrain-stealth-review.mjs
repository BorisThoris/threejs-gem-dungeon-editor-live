...
try{
 ...
 await page.goto('http://127.0.0.1:5217/');await page.locator('[data-testid="menu-start"]').click();
 await page.waitForFunction(()=>window.__run?.getState().phase==='playing'&&!window.__run.getState().transitioning);
 const rows=await page.evaluate(async()=>{
  const din=await import('/src/game/din/din.ts');const {footingCarry,footingAt}=await import('/src/game/rooms/underfoot.ts');
