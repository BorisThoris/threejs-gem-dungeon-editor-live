...
export function skullGeometry(): BufferGeometry {
  ...
  for (let i=0;i<8;i++) { const j=(i+1)%8; indices.push(i,8+i,8+j,i,8+j,j); }
  const indexed = new BufferGeometry();
  indexed.setAttribute("position",new Float32BufferAttribute(positions,3));indexed.setIndex(indices);
  const head=indexed.toNonIndexed();indexed.dispose();head.computeVertexNormals();
  ...
  return merged.translate(0,-.0775,0);
}
...
export function skullSocketsGeometry(): BufferGeometry {
  ...
  for(const x of [-.083,.083]) {
    const a=[x-.042,.235,.106],b=[x+.042,.235,.106],c=[x+.042,.302,.106],d=[x-.042,.302,.106];
