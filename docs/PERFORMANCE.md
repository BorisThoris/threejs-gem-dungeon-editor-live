...
| Area | State | Action |
|---|---|---|
| Finished wood furniture | Stable | Chairs, crates and tables bake their light and dark tints into one wood-grained geometry each. They retain 54, 36 and 44 triangles while reducing a one-of-each furniture set from eight material draws to five. The Turnkeepers' Relay drops from 61 to 59 visible calls at the same 4,764 triangles. The fixed 78-room sweep remains at 78 calls and 7,288 triangles, with 96 live geometries and 11 textures; ten revisits show no growth and a 524-frame sprint retains no heap. Barrels keep a separate iron draw and animated chest lids remain movable. |
| Authored elbow relays | Stable | The L-shaped floor, walls, map, collision and nine furnishings now use one seeded turn and mirror. Across 360 floors, 16 relays occupy all four missing-corner orientations and both mirror states; the direct furnished browser fixture renders at 61 calls and 4,764 triangles. The fixed 78-room sweep remains at 78 calls, 7,288 triangles, 97 geometries and 11 textures. Ten revisits add no geometry, a 579-frame sprint retains no heap, and 20,000 held-audio updates retain one voice with zero bytes per update. |
