...
try{
 ...
 const f=await page.evaluate(async()=>{
  ...
  for(let seed=1;seed<=200;seed++){const dungeon=generateDungeon({seed,floor:2});for(const room of dungeon.rooms.filter(r=>r.kind==='trap')){
   ...
   for(const [index,home] of homes.entries())for(const hazard of hazards){const distance=Math.hypot(hazard.x-home.x,hazard.z-home.z);if(distance<hazard.r+.5||distance>6)continue;
    ...
    if(hit){window.__run.setState({dungeon,floor:2,currentRoomId:room.id,transitioning:false,ratLosses:{},wardenRoomId:null,harrierSlain:true,reaperAwake:false,thiefPhase:'away',invulnerableUntil:1e9});return {id:room.id,other:dungeon.startId,index,px,pz,hazards,home,target:hazard};}
   }
 ...
 // Follow behind the animal: its random initial wander must not decide whether
 // the test's single stationary scare happens to line up with a narrow patch.
 await page.evaluate(async f=>{
  for(let step=0;step<70&&!window.__run.getState().ratLosses[`${f.id}:${f.index}`];step++){
   const rat=window.__rats[f.index],dx=f.target.x-rat.x,dz=f.target.z-rat.z,length=Math.hypot(dx,dz)||1;
   window.__bus.emit('teleport',{position:[rat.x-dx/length*.35,1.5,rat.z-dz/length*.35]});
   await new Promise(resolve=>setTimeout(resolve,100));
  }
 },f);
 assert.ok(await page.evaluate(f=>window.__run.getState().ratLosses[`${f.id}:${f.index}`],f),'herded rat reaches the generated spike patch');
